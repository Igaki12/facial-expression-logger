...
上記の終了導線画面でも、他の節目画面と統一感のある中央アイコン演出を使ってよい。

### 3.3 履歴画面
- `history` では保存済み experiment を一覧表示する
- `history` は HashRouter 経由で `/#/history` に対応させる
...
- 横長画面では履歴画面を 2 列構成にし、一覧列を狭め、プレビュー詳細列を広めにする
- 履歴画面が 1 列構成になる条件では、`history-list` をボトムシート風のモーダルで開く形にしてよい
- 履歴画面が 1 列構成になる条件では、`確認` でプレビューを開いたあと一覧シートを閉じ、`history-detail` に視線を戻す
- 履歴画面を開いた直後は、最新の completed experiment の `確認` が表示された状態を基本にする
- experiment カード内では `録画` と `音声` の見出しを分け、どの操作群か分かるようにする
...
- `録画中` / `録音中` / `顔OK` などの状態バッジは `stage-overlay` より上に重ねてよい
- 状態バッジは動画内下部に置きつつ、`stage-overlay` と間隔を詰めすぎず、視認性を優先する
...
- モバイルの狭い画面では、`camera_check` / `phase_guide` / `phase_recording` 中のレイアウトを簡略化してよい
- 上記 3 画面では、検査と無関係な `story-header` や `progress-strip` を非表示にしてよい
...
- `history` 画面では同じ位置のヘッダーボタンを `戻る` に切り替え、root へ戻す
- 通常画面のヘッダーボタンには right arrow 系の補助アイコンを付けてよい
- `history` 画面のヘッダーボタンには left arrow 系の補助アイコンを付けてよい
- モバイルでヘッダーが折り返す場合も、ヘッダーボタンは右端に寄せる
- `history` 画面内に、ヘッダー導線と重複する単独の `戻る` ボタンを増やさない

### 3.9 ブラウザと終了導線
- `intro` では、LINE などのアプリ内ブラウザを検出した場合に注意モーダルを出してよい
- 上記モーダルでは、Chrome / Safari / Edge など既定ブラウザで開き直す案内を短く出す
- 上記モーダルには、このアプリの URL を表示し、少なくとも `コピー` と `開く` の導線を置く
- 実験完了後は `debrief` の後に `return_guide` 画面を置いてよい
- `return_guide` では、元のページへ戻る案内、戻り先 URL 表示、`開く` / `コピー` の導線を出してよい
- 戻り先 URL は実運用値に差し替え可能な定数で持ってよい。実装中はダミーでもよい
...
- `return_guide` 画面を削除して完了後導線を失わせない
- `intro` のアプリ内ブラウザ注意モーダルを理由なく削除しない
...
- 履歴画面の `/#/history` ルーティングを壊さない
- 横長画面の履歴 2 列レイアウトを 1 列へ戻さない
...
- モバイルの `camera_check` / `phase_guide` / `phase_recording` で簡略化したレイアウトを desktop に波及させない
- モバイルの `camera_check` / `phase_guide` / `phase_recording` で `story-header` と `progress-strip` を常時表示に戻さない
