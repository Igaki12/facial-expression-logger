...
export const RESEARCH_DEBRIEF = {
  title: "この実験で見ていたこと",
  body:
    "この取り組みでは、会話の内容が切り替わるときの表情変化を研究用に記録しています。保存しているのは顔ランドマーク、表情ブレンドシェイプ、顔変換行列などの数値データで、映像ファイルそのものは保存していません。",
  detail:
    "趣味の話と仕事の話を続けて行うことで、話題の違いによる表情の変化を比較できるようにしています。必要があれば、最後の画面から片方だけ撮り直せます。",
};
